 ...
 */
export const BRAND_SORT_WHITELIST = [
  'name:asc',
  'name:desc',
  'createdAt:asc',
  'createdAt:desc',
...
export type BrandSortOption = (typeof BRAND_SORT_WHITELIST)[number];

/**
 * Documented query params only (API_BLUEPRINT.md §6 / AI_RULES.md §30).
 * Any undocumented query param is silently ignored by the global
 * `ValidationPipe` (`whitelist: true`), never causing an error.
 */
export class QueryBrandDto {
  @ApiPropertyOptional({ description: 'Case-insensitive search on brand name.' })
  @IsOptional()
  @IsString()
  search?: string;
